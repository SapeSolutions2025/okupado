...
export async function searchAddresses(query: string): Promise<Address[]> {
  try {
    ...
    if (!reportError && reportAddresses && reportAddresses.length > 0) {
      ...
      // Si no hay resultados en la base de datos, usar datos simulados
      // Si la búsqueda incluye "sol", simular resultados para Puerta del Sol
      if (lowercaseQuery.includes("sol")) {
        cityName = "Madrid"

        results = [
          {
            formattedAddress: "Puerta del Sol, Madrid, España",
            location: {
              lat: 40.4169019,
              lng: -3.7056721,
            },
          ...
          {
            formattedAddress: "Plaza del Sol, 1, Madrid, España",
            location: {
              lat: 40.4165,
              lng: -3.7033,
            },
          ...
          {
            formattedAddress: "Calle del Sol, 15, Madrid, España",
            location: {
              lat: 40.4159,
              lng: -3.7045,
            },
        ...
        const numResults = Math.floor(Math.random() * 5) + 1 // 1-5 resultados

        for (let i = 0; i < numResults; i++) {
          // Variación aleatoria para las coordenadas
          ...
          // Generar código postal con variación
          const basePostalCode = cityName === "Madrid" ? "28001" : "08001"
          const postalCodeVariation = Math.floor(Math.random() * 100)
          const postalCode = (Number.parseInt(basePostalCode) + postalCodeVariation).toString()

          results.push({
            formattedAddress: `${streetName}, ${streetNumber}, ${cityName}, España`,
            location: {
              lat: cityCoordinates[cityName]?.lat + latVariation || 40.4168 + latVariation,
              lng: cityCoordinates[cityName]?.lng + lngVariation || -3.7038 + lngVariation,
            },
            city: cityName,
